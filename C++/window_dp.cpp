...
// Window-based 3D DP-based algorithm
...
tuple<double, double, double> schedule_by_window_dp(vector<double> A_all, vector<double> B_all, vector<double> C_all, int carNum)
{
    ...
    while (A_all.size() > 1 || B_all.size() > 1 || C_all.size() > 1)    // while there is a vehicle
    {
        vector<double> A = get_window_by_num(A_all, carNum);    // get a window of vehicles on Lane A
        vector<double> B = get_window_by_num(B_all, carNum);    // get a window of vehicles on Lane B
        vector<double> C = get_window_by_num(C_all, carNum);    // get a window of vehicles on Lane C
        if (A.size() > 1 && B.size() > 1 && C.size() > 1)   // if no lane is empty
        {
            tie(last_X, last_Y, wait_time) = window_dp_3d(A, B, C, last_X, last_Y); // solve the window
